...
const Recommendations: React.FC = () => {
  ...
  const recommendations = useMemo(() => {
    ...
    map.forEach((data, sku) => {
      ...
      let currentStock;
      if (seed === 0) currentStock = Math.floor(reorderPoint * 0.4); 
      else if (seed === 1) currentStock = Math.floor(reorderPoint * 1.2); 
      else currentStock = Math.floor(reorderPoint * 2.5); 

      const status = currentStock < reorderPoint ? 'Reorder' : 'Healthy';
      const orderQty = Math.ceil(reorderPoint * 2 - currentStock); 
